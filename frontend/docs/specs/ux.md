...
- Dark theme uses neutral black/gray tones rather than tinted colors for reduced eye strain.
- Theme toggle persists preference to localStorage and respects system preference on first visit.
- Implemented via VueUse `useDark()` composable which toggles `.dark` class on `<html>`.

## 9. UX Test Requirements
